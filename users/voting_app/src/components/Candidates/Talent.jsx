...
import { Heart, X, Check, ChevronRight, Instagram, Facebook, Twitter, Menu, Music, Mic } from 'lucide-react';
...
const Talent = () => {
    ...
    const fetchCandidates = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from('candidates')
          .select('*')
          .in('type', ['chanteur', 'danseur']) // On ne récupère que les talents
  ...
  // Confirmer le paiement et générer un coupon
  const confirmPayment = async () => {
    try {
      // Générer le contenu du coupon
      const couponContent = `
=================================================
        COUPON DE PAIEMENT - TALENTS HITBAMAS
=================================================
...
Catégorie: ${selectedCandidate?.type === 'chanteur' ? 'CHANTEUR' : 'DANSEUR'}
...
`;
      ...
      // Insérer les données du vote dans Supabase
      try {
      ...
      // Générer et télécharger le coupon
    ...
    <div className="bg-gradient-to-b from-gray-50 to-white min-h-screen">
        ...
        <div className="container mx-auto">
          ...
          <div className="flex justify-between items-center py-4 px-4">
            <Link to="/" className="text-xl md:text-2xl font-bold flex items-center">
              ...
              <span className="hidden md:inline">TALENTS HITBAMAS</span>
              <span className="md:hidden">HITBAMAS</span>
            </Link>
            
            <nav className="hidden md:flex space-x-6">
              <Link to="/" className="hover:text-[#D6B981] font-medium">Accueil</Link>
              <a href="#chanteurs" className="hover:text-[#D6B981] font-medium">Chanteurs</a>
              <a href="#danseurs" className="hover:text-[#D6B981] font-medium">Danseurs</a>
          ...
          {mobileMenuOpen && (
            <div className="md:hidden bg-[#96172E] py-2 px-4">
              <nav className="flex flex-col space-y-3 pb-4">
                <Link to="/" className="hover:text-[#D6B981] font-medium py-2 border-b border-white/10" onClick={toggleMobileMenu}>Accueil</Link>
                <a href="#chanteurs" className="hover:text-[#D6B981] font-medium py-2 border-b border-white/10" onClick={toggleMobileMenu}>Chanteurs</a>
                <a href="#danseurs" className="hover:text-[#D6B981] font-medium py-2 border-b border-white/10" onClick={toggleMobileMenu}>Danseurs</a>
      ...
      <section className="relative h-64 md:h-80 overflow-hidden">
        <div className="absolute inset-0 bg-black">
          <img 
            src="/hitlogo.jpeg" 
            alt="Talents HITBAMAS Banner" 
            className="w-full h-full object-cover opacity-70"
          />
        </div>
        <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center px-4">
          <h1 className="text-3xl md:text-4xl font-bold mb-4">Talents HITBAMAS 2025</h1>
          <p className="text-lg md:text-xl mb-6 max-w-2xl">Découvrez et votez pour vos talents préférés en chant et danse</p>
        </div>
      </section>
      ...
      {error && (
        <div className="bg-red-50 text-red-800 p-4 rounded-md mx-auto max-w-2xl my-8">
          <p>Une erreur est survenue lors du chargement des talents: {error}</p>
        </div>
      )}
      
      {/* Section Chanteurs */}
      {!loading && chanteurCandidates.length > 0 && (
        <section id="chanteurs" className="py-12 px-4 bg-gray-50">
          <div className="container mx-auto">
            <div className="text-center mb-8">
              <div className="flex items-center justify-center mb-4">
                <Mic size={32} className="text-[#96172E] mr-3" />
                <h2 className="text-2xl md:text-3xl font-bold text-gray-800">Nos Chanteurs 2025</h2>
              </div>
              <p className="text-gray-600 max-w-2xl mx-auto">Découvrez les talents musicaux de HITBAMAS et votez pour vos favoris.</p>
            </div>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              {chanteurCandidates.map((candidate) => (
                <div 
                  key={candidate.id} 
      ...
      {/* Section Danseurs */}
      {!loading && danseurCandidates.length > 0 && (
        <section id="danseurs" className="py-12 px-4 bg-white">
          <div className="container mx-auto">
            <div className="text-center mb-8">
              <div className="flex items-center justify-center mb-4">
                <Music size={32} className="text-[#96172E] mr-3" />
                <h2 className="text-2xl md:text-3xl font-bold text-gray-800">Nos Danseurs 2025</h2>
              </div>
              <p className="text-gray-600 max-w-2xl mx-auto">Découvrez les talents de danse de HITBAMAS et votez pour vos favoris.</p>
            </div>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              {danseurCandidates.map((candidate) => (
                <div 
                  key={candidate.id} 
                  className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow group"
                >
          ...
          <div className="container mx-auto">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
              <div className="p-4 bg-[#D6B981]/10 rounded-lg">
                ...
                <p className="text-gray-600 uppercase font-medium text-sm">Talents</p>
              </div>
              <div className="p-4 bg-[#D6B981]/10 rounded-lg">
            ...
            <div className="p-6">
              ...
              {voteSuccess ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500 text-sm">Présentez-le à un point de paiement pour finaliser votre vote pour Talents HITBAMAS</p>
                </div>
              ) : (
                <>
                  ...
                  <form onSubmit={submitVote}>
                    <div className="space-y-4">
                      ...
                      <div className="text-xs text-gray-500">
                        En votant, vous acceptez les règles du concours Talents HITBAMAS.
                      </div>
              ...
              <div className="space-y-6">
                <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
                  ...
                  <p className="text-sm text-gray-700">
                    ...
                    2️⃣ Payer à l'un des numéros suivants puis présenter votre capture de paiement au responsable de vote :<br />
